...
export default function App() {
  ...
  const [userProfile, setUserProfile] = useState<UserProfile>({
    ...
    degreeLevel: 'بكالوريوس هندسة',
    targetMajor: 'علوم الحاسوب والذكاء الاصطناعي',
    gpa: '3.85 / 4.00 (امتياز)',
    englishLevel: 'IELTS 7.0',
    targetCountries: ['أوروبا', 'بريطانيا', 'ألمانيا', 'الصين'],
  ...
  const [aiToolsInitialTab, setAiToolsInitialTab] = useState<'letter' | 'cv' | 'chat' | 'search'>('letter');
  ...
  const triggerInstantPush = (customNotification?: Partial<PushNotificationItem>) => {
    const newAlert: PushNotificationItem = {
      ...
      body: customNotification?.body || 'تم الإعلان عن منحة ماجستير ودكتوراه ممولة بالكامل في جامعة السوربون بباريس.',
      timestamp: 'الآن',
  ...
  const filteredScholarships = scholarships.filter((s) => {
    ...
    const matchesDegree = selectedDegree === 'الكل' || s.degreeLevel.includes(selectedDegree as any);
    ...
    <div className="flex flex-col min-h-screen w-full bg-slate-50 dark:bg-[#031322] text-slate-900 dark:text-slate-100 selection:bg-[#E4B343]/30 selection:text-slate-950 font-['Cairo',sans-serif] pb-24 sm:pb-28 transition-colors">
      ...
      <main className="w-full mx-auto transition-all flex-1 flex flex-col">
        {selectedScholarship ? (
            ...
            <div className="flex flex-col items-center w-full px-3 sm:px-4 pt-2 sm:pt-3">
              ...
              <div className="w-full relative mt-2 pb-4">
                ...
                <div className="space-y-5 relative z-10 w-full px-1 sm:px-2">
                  ...
                  <div className="relative w-full">
                    <AIToolsBanner
                      onOpenAiTools={() => setActiveTab('ai-tools')}
                    />
                  </div>
          ...
          {(activeTab === 'search' || (activeTab === 'home' && selectedCategory !== 'all')) && (selectedCategory === 'scholarships' || selectedCategory === 'all') ? (
            <ScholarshipsSearchPage 
              ...
              onToggleFavorite={handleToggleFavorite}
            />
          ) : (activeTab === 'search' || (activeTab === 'home' && selectedCategory !== 'all')) && selectedCategory === 'majors' ? (
            <MajorsSearchPage 
              majors={MOCK_MAJORS}
              favoriteIds={favoriteIds}
              onToggleFavorite={handleToggleFavorite}
            ...
            />
          ) : (activeTab === 'search' || (activeTab === 'home' && selectedCategory !== 'all')) && selectedCategory === 'countries' ? (
            <CountriesSearchPage 
            ...
            />
          ) : (activeTab === 'search' || (activeTab === 'home' && selectedCategory !== 'all')) && selectedCategory === 'universities' ? (
            ...
            />
          ) : (activeTab === 'search' || (activeTab === 'home' && selectedCategory !== 'all')) && selectedCategory === 'exams' ? (
            <ExamsSearchPage 
              exams={MOCK_EXAMS}
              onBack={() => {
                setActiveTab('home');
            ...
            />
          ) : (activeTab === 'search' || (activeTab === 'home' && selectedCategory !== 'all')) && selectedCategory === 'courses' ? (
            <CoursesSearchPage 
              courses={MOCK_COURSES}
              onBack={() => {
                setActiveTab('home');
            ...
            <div className="w-full max-w-4xl lg:max-w-5xl mx-auto px-4 py-2 sm:pt-3 space-y-3">
                ...
                {[
                  ...
                  <button
                    ...
                    className={`px-3 py-1.5 rounded-xl text-xs font-bold whitespace-nowrap transition-all ${
                      selectedCategory === item.id || (selectedCategory === 'all' && item.id === 'scholarships')
              ...
              {selectedCategory === 'universities' ? (
                ...
                <div className="pt-2"><FeaturedArticles /></div>
              ) : selectedCategory === 'services' ? (
                <div className="pt-2"><FeaturedServices /></div>
              ) : selectedCategory === 'exams' ? (
                <div className="pt-2"><FeaturedExams /></div>
              ) : selectedCategory === 'jobs' ? (
                <div className="pt-2"><FeaturedJobs /></div>
              ) : selectedCategory === 'tools' ? (
                <div className="pt-2"><AIToolsBanner onOpenAiTools={() => setActiveTab('ai-tools')} /></div>
              ) : (
                ...
                <div className="space-y-3 pt-1">
                  ...
                  <div className="flex items-center justify-between text-xs text-stone-600 font-bold px-1">
                    <span>
                      {filteredScholarships.length} فرصة دراسية متاحة
                    </span>
                    <button
                      onClick={() => {
                    ...
                    filteredScholarships.map((sch) => {
                      ...
                      return (
                        <div
                          ...
                          className="bg-white rounded-2xl border border-stone-200/80 shadow-xs hover:shadow-md transition-all overflow-hidden p-3.5 space-y-2.5 text-right hover:border-amber-400 cursor-pointer active:scale-99"
                        >
                          <div className="flex items-start justify-between gap-3">
                            <div className="flex items-start gap-3">
                              ...
                              <div>
                                <div className="flex items-center gap-1.5">
                                  ...
                                  <span className="text-xs">{sch.countryFlag} {sch.country}</span>
                                </div>
                            ...
                            >
                              <Heart
                                className={`w-4 h-4 ${
                                  isFavorited ? 'fill-amber-400 text-amber-400' : 'text-stone-400'
                                }`}
                              />
                            </button>
                          </div>
            ...
            <div className="w-full max-w-4xl lg:max-w-5xl mx-auto px-4 py-3 space-y-3">
              ...
              {favoriteIds.length === 0 ? (
                <div className="py-16 text-center text-stone-400 text-xs space-y-2">
                  ...
                  <p className="text-[11px] text-stone-500">اضغط على علامة القلب في أي منحة لإضافتها هنا.</p>
                  <button
                  ...
                  {scholarships
                    ...
                    .map((sch) => (
                      <div
                        key={sch.id}
                        onClick={() => setSelectedScholarship(sch)}
                        className="bg-white rounded-2xl border border-amber-200 shadow-xs hover:shadow-md p-3.5 space-y-2 text-right cursor-pointer"
                      >
                        <div className="flex items-start justify-between gap-3">
                        ...
                        <div className="flex items-center justify-between pt-2 border-t border-stone-100 text-[11px]">
                          ...
                          <span className="text-[#002E52] font-bold text-xs">
                            عرض التفاصيل ❯
                          </span>
                        </div>
                      </div>
      ...
      />
        ...
        <NavigationDrawer
          ...
          onNavigate={(target) => {
            setSelectedScholarship(null);
            setSelectedUniversity(null);
            if (target === 'tracker') setActiveTab('tracker');
