...
const CountryFlagImage: React.FC<{ countryId: string; countryName: string; flagEmoji: string; flagUrl?: string }> = ({ 
  countryId, 
  countryName, 
  flagEmoji, 
  flagUrl 
    ...
    <div className="min-h-screen bg-[#f8fafc] text-slate-900 pb-24 font-sans select-none" dir="rtl">
      ...
      <div className="max-w-lg mx-auto px-4 -mt-7 sm:-mt-8 relative z-20">
        <div className="grid grid-cols-2 gap-2 sm:gap-3">
          ...
          <div className="relative bg-white hover:bg-amber-50/40 border-1.5 border-[#D9A93A]/70 hover:border-[#D9A93A] rounded-xl sm:rounded-2xl p-2 sm:p-2.5 flex flex-col items-center justify-center text-center shadow-[0_4px_14px_rgba(0,0,0,0.06)] transition-all h-[58px] sm:h-[64px] cursor-pointer">
            <div className="flex items-center justify-center gap-1.5 text-slate-900 font-extrabold text-xs sm:text-[13px] font-['Cairo',sans-serif] w-full">
              <span className="truncate">{selectedCountryName === 'الكل' ? 'الدولة' : selectedCountryName}</span>
      ...
      <div className="max-w-2xl mx-auto px-1 sm:px-2 pt-3.5 space-y-3">
        ...
        {filteredCountries.length === 0 && (
          <div className="bg-white rounded-2xl p-6 text-center border border-slate-200 shadow-xs space-y-2.5">
            ...
            <h3 className="font-bold text-slate-800 text-xs sm:text-sm">لم يتم العثور على وجهات مطابقة</h3>
          ...
          >
            ...
            <div className="relative bg-gradient-to-r from-[#01241b] via-[#033a2c] to-[#012219] text-white p-5">
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="text-xl font-black font-['Cairo',sans-serif]">{activeCountryModal.name}</h2>
                  <p className="text-xs text-[#E4B343] font-semibold">{activeCountryModal.nameEn} • {activeCountryModal.continent}</p>
                </div>
              </div>
            </div>
            ...
            <div className="p-5 overflow-y-auto space-y-4 font-['Cairo',sans-serif]">
              ...
              <div className="grid grid-cols-2 gap-2.5">
                <div className="p-3 bg-slate-50 rounded-2xl border border-slate-100 space-y-1">
                  <span className="text-[10px] text-slate-400 font-bold block">متوسط تكلفة المعيشة</span>
                  <span className="text-xs font-black text-slate-800">{activeCountryModal.averageLivingCostUsd}</span>
                </div>
                <div className="p-3 bg-slate-50 rounded-2xl border border-slate-100 space-y-1">
                  <span className="text-[10px] text-slate-400 font-bold block">سهولة الحصول على التأشيرة</span>
                  <span className="text-xs font-black text-slate-800">{activeCountryModal.visaEase}</span>
                </div>
                <div className="p-3 bg-slate-50 rounded-2xl border border-slate-100 space-y-1">
                  <span className="text-[10px] text-slate-400 font-bold block">لغة الدراسة الشائعة</span>
                  <span className="text-xs font-black text-slate-800">{activeCountryModal.languageOfStudy.join(' • ')}</span>
                </div>
                <div className="p-3 bg-slate-50 rounded-2xl border border-slate-100 space-y-1">
                  <span className="text-[10px] text-slate-400 font-bold block">المنح المتاحة</span>
                  <span className="text-xs font-black text-[#064D83]">{activeCountryModal.scholarshipsCount} منحة ممولة</span>
                </div>
              </div>
              ...
              <div className="space-y-1.5">
                ...
                <div className="flex flex-wrap gap-1.5">
                  {activeCountryModal.popularCities.map((city) => (
                    <span key={city} className="bg-slate-100 text-slate-700 text-xs px-3 py-1 rounded-full font-semibold">
                      {city}
                    </span>
                  ))}
                </div>
