...
export const MajorCard: React.FC<MajorCardProps> = ({
  ...
  const renderSpecialtyIcon = () => {
    ...
    const nameLower = (major.name + ' ' + (major.nameEn || '') + ' ' + (major.category || '')).toLowerCase();

    if (nameLower.includes('طب') || nameLower.includes('جراحة') || nameLower.includes('medicine') || nameLower.includes('surgery') || nameLower.includes('طبي')) {
      return <Stethoscope className={iconClass} />;
    }
    if (nameLower.includes('قلب') || nameLower.includes('cardio') || nameLower.includes('تمريض') || nameLower.includes('nursing')) {
      return <HeartPulse className={iconClass} />;
    }
    if (nameLower.includes('حاسوب') || nameLower.includes('ذكاء') || nameLower.includes('computer') || nameLower.includes('ai') || nameLower.includes('برمج') || nameLower.includes('tech') || nameLower.includes('cpu')) {
      return <Cpu className={iconClass} />;
    }
    if (nameLower.includes('بيولوج') || nameLower.includes('أحياء') || nameLower.includes('جين') || nameLower.includes('مختبر') || nameLower.includes('microscope')) {
      return <Microscope className={iconClass} />;
    }
    if (nameLower.includes('هندس') || nameLower.includes('عمارة') || nameLower.includes('engineering') || nameLower.includes('civil')) {
      return <Compass className={iconClass} />;
    }
    if (nameLower.includes('قانون') || nameLower.includes('حقوق') || nameLower.includes('شريعة') || nameLower.includes('law')) {
      return <Scale className={iconClass} />;
    }
    if (nameLower.includes('فيزياء') || nameLower.includes('كيمياء') || nameLower.includes('physics') || nameLower.includes('chem') || nameLower.includes('atom')) {
      return <Atom className={iconClass} />;
    }
    if (nameLower.includes('نفس') || nameLower.includes('عصب') || nameLower.includes('psych') || nameLower.includes('neuro')) {
      return <Brain className={iconClass} />;
    }
    if (nameLower.includes('إدار') || nameLower.includes('أعمال') || nameLower.includes('مال') || nameLower.includes('business') || nameLower.includes('finance')) {
  ...
  const primaryDegree = major.degreeLevels && major.degreeLevels.length > 0 
    ? major.degreeLevels[0] 
    : 'بكالوريوس';
